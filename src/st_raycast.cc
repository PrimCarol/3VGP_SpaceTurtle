...
glm::vec3 ST::Raycast::ScreenPosToWorldRay(const ST::Window& w, const ST::Camera& cam){
	...
	glm::vec4 lRayStart_NDC(
		((float)w.mousePosX() / (float)w.getWindowsWidth() - 0.5f) * 2.0f, // [0,1024] -> [-1,1]
		((float)w.mousePosY() / (float)w.getWindowsHeight() - 0.5f) * 2.0f, // [0, 768] -> [-1,1]
		-1.0, // The near plane maps to Z=-1 in Normalized Device Coordinates
		1.0f
	);
	glm::vec4 lRayEnd_NDC(
		((float)w.mousePosX() / (float)w.getWindowsWidth() - 0.5f) * 2.0f,
		((float)w.mousePosY() / (float)w.getWindowsHeight() - 0.5f) * 2.0f,
		0.0,
		1.0f
	);
